...
// the CloudFront function deployed from the infra repo. A binary pointing at a
// host the function no longer maps has no upgrade path back, so these tests pin
// the two halves that live here to each other rather than to a literal.
