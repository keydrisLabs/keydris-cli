...
- **Theft equals impersonation.** A co-resident process that reads the token — from the environment or from `$CLAUDE_ENV_FILE` — can impersonate the session until `session-end`. Mitigations: short token lifetime, tighter `$CLAUDE_ENV_FILE` permissions, and moving to kernel-asserted attribution so identity is *observed* rather than *claimed* (Tiers 2 and 3 in [docs/attribution.md](docs/attribution.md)).
