...
New to the codebase? Read [ARCHITECTURE.md](ARCHITECTURE.md) first. It traces one request end to end and names the
file that owns each step, which is faster than reading `internal/` in alphabetical order.
...
| [`docs/`](docs/) | Focused guides: sandbox, attribution, codex, releasing, npm distribution |
| [`ARCHITECTURE.md`](ARCHITECTURE.md) | The design and operational reference for all of the above |
...
Node 20 or newer. Every script uses only Node's standard library; generated executables are gitignored. Details in
...
[`release.yml`](.github/workflows/release.yml) publishes on tag push `v*` (→ `stable`) and push to `main` (→ `dev`).
There is no separate pull-request workflow yet, so **the bar is what you run locally**:
...
| `channelBaseURL` in [`upgrade.go`](internal/cli/upgrade.go) | `scripts/render-install.sh` — [`channel_binding_test.go`](internal/cli/channel_binding_test.go) pins them to each other, and `CHANNEL_BY_HOST` in the CloudFront function lives in the infra repo where nothing here can check it |
| A wire shape in `internal/runtimecontract` | The corresponding JSON block in [README.md](README.md) and the walkthrough in [ARCHITECTURE.md](ARCHITECTURE.md) |
| A routing branch in `runtime_router.go` | The decision table in [ARCHITECTURE.md](ARCHITECTURE.md) §11 |
| What `init` writes | [`examples/claude-code/settings.json`](examples/claude-code/settings.json) or [`examples/codex/`](examples/codex/), and [docs/sandbox.md](docs/sandbox.md) or [docs/codex.md](docs/codex.md) |
...
5. Write a commit message that says what changed and why; the history uses `feat:` / `fix:` / `chore:` / `ci:`
   prefixes and references the tracking issue where there is one (`(ENG-224)`).
...
agree on the shape before you build it. [ARCHITECTURE.md §14](ARCHITECTURE.md#14-extension-points) describes the
seams those changes are meant to plug into.
...
```bash
git tag v0.1.0 && git push origin v0.1.0     # -> stable channel + npm `latest`
```

Pushing to `main` refreshes the `dev` channel and publishes an npm prerelease under `next` automatically. Both paths
cross-compile the matrix, verify checksums, sync to S3, invalidate CloudFront, publish the six native npm packages,
wait for each to be visible in the registry, and publish `@keydris/cli` last. AWS and npm both authenticate through
GitHub OIDC — there is no AWS key and no npm token.

Note that **both installer flavors publish on every run**, because the installer is bound to a channel by its host,
not by the channel being released. An installer change therefore reaches stable users as soon as it lands on `main`.
The full runbook, including the infrastructure prerequisites and the rollout ordering, is in
